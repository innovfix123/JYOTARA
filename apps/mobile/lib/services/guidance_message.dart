...
ChatMessage guidanceMessage(GuidanceResponse result, String language) {
  ...
  final indiaTime = result.answeredAt?.toUtc().add(const Duration(hours: 5, minutes: 30));
  final timestamp = indiaTime?.toIso8601String().substring(0, 16).replaceFirst('T', ' ');
  final heading = limited
      ? copy(
  ...
  return ChatMessage(
    ...
    text: [
      ...
      if (timestamp != null) '${copy('Answer prepared', 'பதில் தயாரிக்கப்பட்ட நேரம்', 'Answer thayaaricha neram')}: $timestamp IST',
      result.answer,
    ...
    label: result.answerMode == 'practical_guidance'
        ? copy('PRACTICAL GUIDANCE', 'நடைமுறை வழிகாட்டல்', 'NADAIMURAI GUIDANCE')
        : traditional
        ? copy('TRADITIONAL GUIDANCE', 'பாரம்பரிய வழிகாட்டல்', 'PARAMBARIYA GUIDANCE')
        : limited
        ? copy(
            'LIMITED GUIDANCE',
