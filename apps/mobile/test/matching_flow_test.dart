...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
                ...
                onPressed: () => Navigator.of(context).push(
                  MaterialPageRoute<void>(
                    builder: (_) => BirthForm(
                      ...
                      initialGender: ProfileGender.male,
                      initialDetails: {
      ...
      await tester.ensureVisible(find.text('Use these birth details'));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'matching button gives visible feedback when both profiles are missing',
    (tester) async {
        ...
        MaterialApp(
          home: MatchingScreen(
            request: (path, body) async {
              calls++;
              submitted = body;
              expect(path, '/api/kundli/matching');
              return {
                'score': 24.5,
                'maximum': 36,
                'interpretation': 'Test chart comparison.',
                'note': 'Traditional interpretation.',
                'source': 'Fixture',
              };
            },
          ),
  ...
  testWidgets('unknown birth times submit and show a provisional result', (
    tester,
  ) async {
    var calls = 0;
    Map<String, dynamic>? submitted;
