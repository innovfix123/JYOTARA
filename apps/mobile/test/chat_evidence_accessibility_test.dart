...
void main() {
  testWidgets(
    'limited Tamil reading keeps its evidence complete and selectable at enlarged text',
    (tester) async {
      ...
      final facts = List.generate(
        19,
        (i) => 'சோதனை விவரம் ${i + 1}: கணக்கிடப்பட்ட தகவல்',
      );
      ...
      expect(
        find.ancestor(
          of: find.text(message.text),
          matching: find.byType(SelectionArea),
        ),
        findsOneWidget,
      );
      expect(
        tester.widget<Text>(find.text(message.label!)).style?.fontSize,
        14,
      );
      await tester.drag(
        find.byKey(const Key('chatHistoryList')),
