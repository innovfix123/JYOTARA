...
void main() {
  test('bounded guide context survives retry and restart, and clears with profile', () async {
    String? disk;
    bool fail = true;
    ...
    ProfileSession make() => ProfileSession(vault: vault, api: JyotaraApiClient(
      baseUrl: 'https://example.test', client: MockClient((request) async {
        ...
        if (fail) throw http.ClientException('Synthetic interrupted delivery');
        return http.Response('{"profileId":"one","answer":"Practical response","evidence":[],"answerMode":"practical_guidance"}', 200);
      }),
    ));
    var session = make();
    ...
    for (var i = 0; i < 8; i++) {
      chat.messages.add(ChatMessage(fromUser: true, text: 'Prior statement $i'));
      chat.messages.add(const ChatMessage(fromUser: false, text: 'Old unverified assistant claim'));
    }
    chat.messages.add(const ChatMessage(fromUser: true, text: 'Another chance?'));
    chat.pending = true;
    chat.changed();
    session.conversation('Arivan').messages.add(const ChatMessage(fromUser: true, text: 'Private career question'));
    Future<GuidanceResponse> ask(ProfileSession value, [String guide = 'Aadhirai']) => value.ask(
      category: 'Love', question: 'Another chance?', responseStyle: 'english', guide: guide);
    await expectLater(ask(session), throwsA(isA<JyotaraApiException>()));
    expect(sent.single['previousUserMessages'], List.generate(6, (i) => 'Prior statement ${i + 2}'));
    expect(jsonEncode(sent.single), isNot(contains('Old unverified')));
    ...
    session.conversation('Aadhirai').messages.add(const ChatMessage(fromUser: true, text: 'New statement after uncertain send'));
    fail = false;
    ...
    await session.recordGuidanceResponse(reply, session.conversation('Aadhirai'), 'english');
