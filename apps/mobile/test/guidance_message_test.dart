...
void main() {
  ...
  test('recovered answers retain original server time and are not labelled fresh', () {
    final response = GuidanceResponse.fromJson({
      'answer': 'Saved answer', 'evidence': [], 'answerMode': 'grounded_fallback',
      'replayed': true, 'answeredAt': '2026-09-07T18:45:00Z',
    });
    ...
    expect(guidanceMessage(response, 'tamil').text, contains('முன்பு தயாரித்த பதில்'));
    expect(RegExp(r'[\u0B80-\u0BFF]').hasMatch(guidanceMessage(response, 'tanglish').text), false);
    final legacy = GuidanceResponse.fromJson({'answer': 'Legacy saved answer', 'evidence': [], 'replayed': true});
    expect(guidanceMessage(legacy, 'english').text, isNot(contains('Answer prepared')));
    for (final change in [
      {'replayed': 'true'}, {'answeredAt': '2026-02-30T12:00:00Z'},
      {'answeredAt': '2026-09-07T10:00:00'}, {'answeredAt': 123},
    ]) {
      expect(() => GuidanceResponse.fromJson({'answer': 'Answer', 'evidence': [], ...change}), throwsA(isA<JyotaraApiException>()));
    }
  });
  test('reviewed traditional mode retains the distinction from guaranteed predictions', () {
    for (final language in ['english', 'tamil', 'tanglish']) {
      final message = guidanceMessage(response('reviewed_traditional'), language);
      expect(message.text, contains('Original answer'));
      expect(message.text, contains('Original limitation'));
      expect(message.label, isNot('PERSONALISED GUIDANCE'));
    }
    expect(guidanceMessage(response('reviewed_traditional'), 'english').label, 'TRADITIONAL GUIDANCE');
  });
}
