type Credentials = { PROKERALA_CLIENT_ID?: string; PROKERALA_CLIENT_SECRET?: string };
...
export async function prokeralaJson(credentials: Credentials, path: string, input: {
  ...
  const response = await fetch(url, { signal: AbortSignal.timeout(12_000), redirect: 'manual', headers: { Authorization: `Bearer ${token}` } });
