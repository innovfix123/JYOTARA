...
// Local QA fixture: do not copy birth records into source, logs or public assets.
// Intentionally fail if the retained fixture is unavailable, rather than skip.
const fixture = JSON.parse(readFileSync(process.env.JYOTARA_PRIVATE_QA_DIR + '/results.json', 'utf8'));
...
test('all three retained provider charts pass the structural boundary', () => {
...
test('retained undated context is not relabelled current by new evidence code', () => {
...
test('retained occupied-tenth charts do not receive the empty-tenth vocational reading', () => {
  for (const [index, profile] of fixture.profiles.entries()) {
    ...
    const check = packet => {
      ...
      const result = reviewedCareerResponse({
        snapshotId: `retained-${index + 1}`, questionId: 'offline-career-coverage', packet, style: 'english',
      });
    ...
    assert.equal(profile.facts.navamsa?.length ?? 0, 0, 'historical records have no retained D9');
...
test('retained occupied charts receive separate support readings without inventing D9 or job predictions', () => {
  for (const [index, profile] of fixture.profiles.entries()) {
    ...
    const evaluate = packet => reviewedCareerResponse({snapshotId: `retained-${index + 1}`, questionId: 'support', packet, style: 'english'});
...
test('rejected Tamil and Tanglish support tone stays inactive while English remains available', () => {
  ...
  for (const [index, profile] of fixture.profiles.entries()) {
    for (const style of ['english', 'tamil', 'tanglish']) {
      ...
      const result = reviewedCareerResponse({snapshotId: `retained-${index}`, questionId: 'support-language', packet, style});
      if (style !== 'english') {
